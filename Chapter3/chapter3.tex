...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1.2cm, auto, scale=0.8, transform shape]
		...
		\node (md5) [process, right=1.0cm of in] {Tính toán mã băm MD5};
		\node (check) [decision, right=1.0cm of md5] {MD5 đã tồn tại?};
		
		\node (stop) [startstop, right=1.0cm of check] {Bỏ qua (Trùng lặp)};
		\node (chunk) [process, below=1.1cm of check] {Chuyển sang phân đoạn \\ (Chunking)};
...
\begin{figure}[H]
	\centering
	\vspace{-0.6cm}
	\begin{tikzpicture}[node distance=1.2cm, auto, scale=0.78, transform shape]
		% Nodes
		\node (input) [startstop] {URL Mục tiêu};
		\node (dom) [process, right=1.0cm of input] {Nhận diện cấu trúc DOM};
		\node (check) [decision, right=1.0cm of dom, text width=3cm] {Đã tồn tại trong \\ Registry hệ thống?};
		
		% KHÔNG (Mới) branch
		\node (llm) [process, below=1.2cm of dom, text width=3.5cm] {Phân tích LLM \\ (Logic tô pô DOM)};
		\node (gen) [process, left=1.0cm of llm, text width=3.2cm] {Tạo bộ chọn \\ CSS/JS tối ưu};
		\node (save) [process, below=1.2cm of gen, text width=3.5cm] {Cập nhật Registry \\ (Lưu trữ bộ chọn)};
		
		% CÓ (Đã biết) branch
		\node (exe) [process, below=1.2cm of check, text width=4cm] {Thực thi Scraper Engine \\ (Playwright \& Stealth)};
		\node (clean) [process, below=1.2cm of exe, text width=3.2cm] {Làm sạch Markdown};
		
		% Terminal node
		\node (out) [startstop] at (dom |- save) {Đầu ra Markdown cuối cùng};
		...
		% Split from check
		\draw [arrow] (check) -- node [above, sloped, font=\footnotesize] {KHÔNG (Mới)} (llm);
		\draw [arrow] (check) -- node [left, font=\footnotesize] {CÓ (Đã biết)} (exe);
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1.5cm, auto, scale=0.85, transform shape]
		...
		\node (prompt) [process, right=1.2cm of input] {Tạo Prompt trích xuất \\ (NER + RE Schema)};
		\node (llm1) [process, right=1.2cm of prompt] {LLM Trích xuất lần 1};
		
		% Gleaning loop nodes
		\node (check) [decision, below=1.6cm of llm1] {Còn tri thức sót?};
		\node (llm_extra) [process, below=1.0cm of check] {Trích xuất bổ sung};
		
		% Coordinate for loop path to fit in the box
		\coordinate (loop_turn) at ($(llm_extra.east) + (1.2cm, 0)$);
		
		% Group box for Gleaning loop
		\begin{scope}[on background layer]
			\node (loop) [groupbox, fit=(check) (llm_extra) (loop_turn), inner sep=12pt] {};
			\node [above right=1mm of loop.north west, anchor=south west, font=\small\bfseries] {Quy trình Gleaning (Thu nhặt bổ sung)};
		\end{scope}
		
		% Final output node
		\node (final) [startstop] at (input |- check) {Danh sách Thực thể \\ \& Quan hệ cuối cùng};
		...
		% Loop back arrow from llm_extra to check
		\draw [arrow] (llm_extra.east) -- ++(1.0cm,0) |- (check.east);
		
		% Arrow from check to final
		\draw [arrow] (check) -- node[anchor=south, font=\footnotesize] {KHÔNG} (final);
...
\begin{figure}[H]
	\centering
	\vspace{-0.4cm}
	\begin{tikzpicture}[node distance=1.2cm, auto, scale=0.72, transform shape]
		% Nodes
		\node (in) [startstop, text width=5.2cm] {Thực thể \& Quan hệ trích xuất \\ \footnotesize(theo từng phân đoạn)};
		\node (g1) [decision, right=2.0cm of in, text width=2.4cm] {chunk\_id\\tồn tại?};
		\node (proceed) [smallbox, right=2.2cm of g1, yshift=1.4cm, text width=3.6cm] {PROCEED \\ \footnotesize(phân đoạn mới, tính bậc cộng dồn)};
		\node (reingest) [smallbox, right=2.2cm of g1, yshift=-1.4cm, text width=3.6cm] {RE\_INGEST \\ \footnotesize(đã tồn tại: nạp lại, tính bậc theo max)};

		% Groupbox Row 2
		\node (g2box) [groupbox, below=3.8cm of g1, xshift=0.9cm, minimum width=11.4cm, minimum height=2.6cm] (g2) {};
		\node [above right=1mm of g2.north west, anchor=south west, font=\small\bfseries] {Kiểm tra cấu trúc \& Đối chiếu bằng chứng};
		\node (shapes) [smallbox, text width=4.6cm] at ($(g2.center)+(2.8cm,0)$) {SHACL Shapes \\ \footnotesize(name, type, isolated, provenance, weight, foreign-script)};
		\node (ground) [smallbox, text width=4.6cm] at ($(g2.center)+(-2.8cm,0)$) {Grounding xác định \\ \footnotesize(exact + fuzzy $\ge$ 0.9, span bằng chứng)};

		\node (dec) [decision, left=0.8cm of g2, text width=3.2cm] {\texttt{shacl\_pass} \& grounded?};

		% Row 3
		\node (approved) [startstop, below=1.8cm of dec, text width=2.8cm, fill=white] {APPROVED \\ \footnotesize(Hợp nhất Đồ thị)};
		\node (pending) [startstop, right=2.2cm of approved, text width=3cm, fill=white] {PENDING\_REVIEW \\ \footnotesize(Gắn cờ shacl\_review)};
		\node (rejected) [startstop, right=2.2cm of pending, text width=2.8cm, fill=white] {REJECTED \\ \footnotesize(Loại bỏ)};
		...
		% Arrows
		\draw [arrow] (in) -- (g1);
		\draw [arrow] (g1.north) |- node[anchor=south, pos=0.75, font=\footnotesize] {KHÔNG} (proceed.west);
		\draw [arrow] (g1.south) |- node[anchor=north, pos=0.75, font=\footnotesize] {CÓ} (reingest.west);
		
		% Route proceed around reingest to avoid crossing it
		\coordinate (gap) at ($(in.east)!0.4!(g1.west)$);
		\coordinate (y_top) at ($(proceed.north) + (0,0.5)$);
		\coordinate (y_mid) at ($(g1.south)!0.5!(g2.north)$);
		
		\coordinate (top_left) at (gap |- y_top);
		\coordinate (bottom_left) at (gap |- y_mid);
		\coordinate (bottom_right) at (ground.north |- y_mid);
		
		\draw [arrow] (proceed.north) |- (top_left) -- (bottom_left) -- (bottom_right) -- (ground.north);
		\draw [arrow] (reingest.south) -- ++(0,-0.4) -| (shapes.north);
		
		\draw [arrow] (g2.west) -- (dec.east);
		
		% Tree-like branching to avoid crossing the groupbox
		\coordinate (mid) at ($(dec.south) + (0,-0.8)$);
		\draw [arrow] (dec.south) -- node[anchor=east, font=\footnotesize] {Đủ bằng chứng} (approved.north);
		\draw [arrow] (mid) -| node[anchor=south, pos=0.4, font=\footnotesize] {Thiếu BC, LLM$\ge$0.8} (pending.north);
		\draw [arrow] (mid) -| node[anchor=south, pos=0.75, font=\footnotesize] {Lỗi cấu trúc} (rejected.north);
...
\centering
\begin{tikzpicture}[
    node distance=1.2cm, auto, scale=0.8, transform shape,
...
\node[inputbox] (input) {%
    Thực thể đầu vào\\
    \small\{tên, mô tả, loại\}
};

\node[stagebox, right=1.0cm of input] (s1) {%
    Giai đoạn 1: Candidate Retrieval\\
    \footnotesize(Truy vấn Vector Store\\tìm top-K láng giềng $\ge \tau$)
};

\node[decisionbox, right=1.0cm of s1] (dec) {
    Tìm được\\ứng viên\\tiềm năng?
};

\node[skipbox, right=1.0cm of dec] (skip) {%
    Thực thể mới\\hoàn toàn\\$\rightarrow$ Tạo nút mới
};

%% Row 2
\node[stagebox, below=1.8cm of dec] (s2) {%
    Giai đoạn 2: LLM Decision\\
    \footnotesize(LLM phân tích ngữ nghĩa sâu\\và đưa ra phán quyết)
};

\node[outputbox, left=1.2cm of s2] (output) {%
    Kết quả phân giải:\\
    Ánh xạ đồng nhất\\hoặc tạo nút độc lập
};
...
% Route skip around s2 to output.south
\draw[dasharrow, rounded corners] (skip.south) |- ($(output.south) + (0,-0.6)$) -- (output.south);
