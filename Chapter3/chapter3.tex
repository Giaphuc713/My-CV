...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1.2cm, auto, scale=0.8, transform shape]
		...
		\node (staging) [smallbox, below=1.2cm of pending, text width=4cm, fill=orange!8] {Staging Area \\ \footnotesize(Giao diện Admin \& API rà soát)};
...
\begin{figure}[H]
	...
	\resizebox{\textwidth}{!}{
		\begin{tikzpicture}[
			...
			ragSystemBox/.style={base, rectangle, rounded corners=5pt, fill=green!15, draw=green!70!black, minimum width=5.5cm, minimum height=1.5cm},
			...
			\begin{scope}[on background layer]
				% 0. Pre-Process Group
				\filldraw[fill=blue!5, draw=blue!40, dashed, rounded corners=15pt, very thick] (-15, 6.8) rectangle (16, 0.5);
				\node[groupLabel, text=blue!70!black] at (-14.5, 6.8) {0. Phân tích \& Định tuyến (Analyzer Agent)};
				
				% 1. Retrieve Group 
				\filldraw[fill=green!5, draw=green!40, dashed, rounded corners=15pt, very thick] (-15, 0) rectangle (16, -4.5);
				\node[groupLabel, text=green!70!black] at (-14.5, 0) {1. Truy xuất Ngữ cảnh (LightRAG System)};
				
				% 2. Generation Group
				\filldraw[fill=yellow!5, draw=yellow!40, dashed, rounded corners=15pt, very thick] (-15, -5) rectangle (16, -7.5);
				\node[groupLabel, text=yellow!70!black] at (-14.5, -5) {2. Tạo sinh câu trả lời (Generator Agent)};
				
				% 3. Memory Group
				\filldraw[fill=purple!5, draw=purple!40, dashed, rounded corners=15pt, very thick] (-1, -8) rectangle (16, -11);
				\node[groupLabel, text=purple!70!black] at (-0.5, -8) {3. Cập nhật Bộ nhớ (Memory Agent)};
			\end{scope}
			...
			\node[labelTag, text=orange!90!red] at (11, 3.2) {decision == 'direct\_answer'};
			...
			\node[labelTag, text=red] at (11, 3.9) {decision == 'reject'};
...
Cơ chế truy xuất hai luồng cục bộ và toàn cục cùng quy trình lọc ngữ cảnh của LightRAG được thể hiện chi tiết ở Hình~\ref{fig:lightrag_retrieval_internal}.
 ...
 \begin{figure}[H]
	...
	\begin{tikzpicture}[
	    >=Stealth,
	    every node/.style={font=\sffamily\small},
	    storageBox/.style={rectangle, rounded corners=3pt, draw=green!70!black, fill=green!10, thick, align=center, text width=4.8cm, minimum height=1.0cm},
	    nodeBox/.style={rectangle, rounded corners=3pt, draw=gray!60, fill=white, thick, align=center, text width=3.8cm, minimum height=0.8cm},
	    inputBox/.style={rectangle, rounded corners=3pt, draw=purple!80!black, fill=purple!10, thick, align=center, text width=3.2cm, minimum height=0.8cm},
	    outputBox/.style={rectangle, rounded corners=3pt, draw=teal!80!black, fill=teal!15, thick, align=center, text width=3.2cm, minimum height=0.8cm},
	    ...
	    %% KHUNG RETRIEVE
	    \filldraw[fill=pink!8, draw=pink!40, dashed, rounded corners=12pt, very thick] (-18.0, 6.8) rectangle (18.0, -1.1);
	    \node[font=\sffamily\bfseries\large, anchor=north west, color=pink!70!black] at (-17.7, 6.5) {Retrieve (Phân hệ Truy xuất)};
	    ...
	    %% KHUNG GENERATION
	    \filldraw[fill=yellow!8, draw=yellow!40, dashed, rounded corners=12pt, very thick] (-5.8, -1.4) rectangle (5.8, -5.5);
	    \node[font=\sffamily\bfseries\large, anchor=north west, color=yellow!70!black] at (-5.5, -1.7) {Generation (Tạo sinh)};
	    ...
	    \draw[solidEdge] (SysPromptExt.north) -- node[labelTag, text=green!60!black, fill=pink!8, pos=0.5] {use LLM} (KWExtract.south);
	    ...
	    %% E. LowKW -> Embed1
	    \draw[solidEdge] (LowKW.north)  -- node[labelTag, text=red!80!black, fill=pink!8, pos=0.5] {use EB Model} (Embed1.south);

	    %% F. HighKW -> Embed2
	    \draw[solidEdge] (HighKW.north) -- node[labelTag, text=red!80!black, fill=pink!8, pos=0.5] {use EB Model} (Embed2.south);
...
Việc viết lại câu hỏi thành dạng độc lập kết hợp với từ khóa giúp khắc phục hiện tượng mất ngữ cảnh khi câu hỏi của sinh viên quá ngắn hoặc mơ hồ. Toàn bộ trạng thái hội thoại được lưu bền vững qua cơ chế checkpoint trên MongoDB, cho phép khôi phục đúng bước đang xử lý khi có gián đoạn.
