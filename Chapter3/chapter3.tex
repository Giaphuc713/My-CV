...
\begin{itemize}
	\item \textbf{Môi trường phát triển và thử nghiệm (Local Deployment):} Mô hình cho phép tải trực tiếp về phần cứng cá nhân thông qua thư viện sentence-transformers và thực hiện xử lý vector hóa offline \cite{multilingual_e5}. Cơ chế này mang lại tính độc lập và tính toàn vẹn cao (Self-contained) cho đồ án. Khi hội đồng hoặc giảng viên sao chép mã nguồn, hệ thống có thể biên dịch và chạy ngay lập tức mà không cần kết nối mạng bên ngoài, không tốn chi phí duy trì token và loại bỏ sự phụ thuộc vào API Key của bên thứ ba \cite{multilingual_e5}.
	\item \textbf{Môi trường vận hành thực tế (Production Hosting API):} Để đưa hệ thống ra thị trường phục vụ hàng ngàn sinh viên truy cập đồng thời, hệ thống chuyển sang mô hình Self-hosted API độc lập. Mô hình nhúng được đóng gói và triển khai thành một vi dịch vụ (Microservice) riêng biệt trên máy chủ đám mây (Cloud) có hỗ trợ GPU tăng tốc thông qua các framework inference chuyên dụng như TEI (Text Embeddings Inference) của Hugging Face hoặc Triton Inference Server của NVIDIA. Kiến trúc này kích hoạt tính năng xử lý hàng đợi động (Dynamic Batching) và lượng hóa ma trận về định dạng Float16, giúp nén tối đa độ trễ sinh vector nhúng thời gian thực xuống dưới 10 mili giây (ms), đáp ứng trọn vẹn áp lực tải diện rộng trong Production.
\end{itemize}
...
\indent Phân hệ di động được phát triển độc lập trên nền tảng framework Flutter và ngôn ngữ lập trình Dart, đóng vai trò là lớp Client di động kết nối đồng bộ dữ liệu với hệ thống máy chủ trung tâm qua các RESTful API. Kiến trúc mã nguồn ứng dụng mô hình quản lý trạng thái tập trung thông qua bộ thư viện Provider (gồm MultiProvider và ChangeNotifierProvider) nhằm tách biệt hoàn toàn tầng giao diện (UI) và logic xử lý nghiệp vụ. Do các chức năng nghiệp vụ chính tương đồng với phân hệ Web, phân hệ di động tập trung triển khai và cấu hình các thành phần kỹ thuật đặc thù trên thiết bị di động bao gồm:

\begin{itemize}
	\item \textbf{Thành phần trợ lý ảo AI:} Hiện thực hóa lớp ChatbotScreen kết nối với máy chủ LightRAG. Phân hệ tích hợp giải pháp xử lý nối chuỗi văn bản (Context Concatenation) để bảo toàn bối cảnh cuộc hội thoại và sử dụng bộ dịch cú pháp Markdown để hiển thị định dạng câu trả lời từ mô hình ngôn ngữ lớn trực quan trên giao diện RichText.
	\item \textbf{Thành phần xác thực tài khoản:} Quản lý toàn bộ vòng đời đăng nhập, đăng ký và làm mới mã truy cập ngầm định (refreshAccessToken) qua Bearer JWT Token. Phân hệ di động cấu hình bộ lọc tự động bắt ngoại lệ tầng kết nối mạng (SocketTimeoutException) tại tính năng khôi phục mật khẩu, hỗ trợ luồng mã Token giả lập "DEMO123" nhằm đảm bảo tính ổn định tối đa cho chu trình thử nghiệm đồ án.
	\item \textbf{Thành phần đồng bộ và tải tệp tin:} Xử lý hiển thị động các thông báo chưa đọc (Badge) trên thanh công cụ CustomAppBar bằng widget Stack. Đồng thời, phân hệ hiện thực hóa luồng đóng gói tệp tin hình ảnh nhị phân đa phần (MultipartFile) từ bộ nhớ máy thông qua thư viện ImagePicker để đồng bộ trực tiếp lên hạ tầng lưu trữ đám mây qua lệnh gọi http.MultipartRequest.
\end{itemize}
...
% --- Màn hình 1 ---
\subsubsection{Màn hình AI Chatbot - Cố vấn học tập thông minh}
