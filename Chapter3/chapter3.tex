...
- Quản lý trạng thái và bối cảnh hội thoại cục bộ: Tích hợp lớp SplashScreen để tự động điều phối phân ranh luồng hiển thị giao diện giữa Chế độ khách (Guest Mode) và Chế độ thành viên (Authenticated Mode). Hệ thống thực hiện lưu trữ phiên và nén chuỗi ngữ cảnh tin nhắn liền kề (Context Concatenation) trước khi gọi API LightRAG, đồng thời hiện thực bộ dịch cú pháp Markdown dạng RichText bằng Regex để kết xuất bối cảnh hiển thị câu trả lời từ LLM.

- Đóng gói dữ liệu nhị phân đa phần (Multipart Request): Định nghĩa luồng truyền tải tệp tin hình ảnh từ bộ nhớ thiết bị thông qua gói thư viện ImagePicker kết hợp với kiểu dữ liệu MultipartFile, cho phép truyền tải bất đồng bộ luồng nhị phân để cập nhật ảnh đại diện sinh viên trực tiếp lên hạ tầng lưu trữ đám mây qua phương thức PUT của http.MultipartRequest.

\clearpage
\subsection{Thiết kế giao diện}
\subsubsection{Giao diện phân hệ ứng dụng di động (Mobile Application)}
...
% --- Màn hình 1 ---
\subsubsubsection{Màn hình AI Chatbot}
...
\indent Mục đích: Cung cấp không gian tương tác hội thoại trực tiếp giữa sinh viên và AI Agent để tra cứu môn học, tư vấn lộ trình đào tạo cá nhân hóa và giải đáp các thắc mắc về học chế tín chỉ.

\indent Mô tả bố cục giao diện: Màn hình sử dụng cấu trúc hiển thị dòng thời gian tin nhắn tăng dần theo chiều dọc. Thanh nhập liệu được cố định chắc chắn ở đáy màn hình tích hợp cơ chế tự động thu nhỏ vùng hiển thị khi bàn phím ảo xuất hiện. Ở chế độ Thành viên, một trình đơn kéo Sidebar dạng Drawer được tích hợp ẩn ở cạnh trái, cho phép người dùng chạm để mở rộng danh sách các phiên hội thoại cũ.

\indent Cách sử dụng:

- Người dùng tiến hành nhập câu hỏi hoặc yêu cầu tư vấn học tập vào khung văn bản ở đáy màn hình.

- Chọn nút Gửi. Hệ thống AI Agent phân tích dữ liệu thực tế, truy xuất đồ thị tri thức LightRAG và thực hiện hiển thị câu trả lời tức thời dạng dòng (streaming).

- Ở trạng thái đã đăng nhập (Thành viên), nội dung cuộc chuyện sẽ được ghi lại và hiển thị trên menu Drawer để sinh viên có thể xem lại.

% --- Màn hình 2 ---
\subsubsubsection{Màn hình quản lý tiến trình học tập}
...
\indent Mục đích: Giúp người học theo dõi trực quan tổng số tín chỉ học phần tích lũy hiện hành dựa trên cột mốc giới hạn 138 tín chỉ tốt nghiệp bắt buộc của Trường Đại học Khoa học Tự nhiên và quan sát danh sách lộ trình gợi ý học tập tiếp theo được cá nhân hóa do AI Agent đề xuất.

\indent Mô tả bố cục giao diện: Màn hình được tổ chức theo cấu trúc các khối chức năng xếp chồng theo một cột dọc duy nhất. Khu vực phía trên hiển thị biểu đồ thanh ngang Linear Progress Bar phản ánh phần trăm hoàn thành chặng đường tích lũy tín chỉ tốt nghiệp hiện tại của sinh viên. Khu vực phía dưới là một danh sách cuộn bao gồm các thẻ môn học (cards) được AI gợi ý riêng biệt để đăng ký trong học kỳ kế tiếp.

\indent Cách sử dụng:

- Sinh viên truy cập vào màn hình Trang chủ (Home) để kiểm tra nhanh thanh tiến độ tích lũy tín chỉ học tập tổng quan của mình.

- Hệ thống tự động phân tích dữ liệu điểm số từ cơ sở dữ liệu PostgreSQL để kết xuất đồ thị thanh ngang phản ánh phần trăm hoàn thành lộ trình đào tạo.

- Sinh viên cuộn xuống dưới để xem chi tiết các thẻ khuyến nghị môn học và lộ trình học tập tối ưu tiếp theo.

% --- Màn hình 3 ---
\subsubsubsection{Màn hình hồ sơ cá nhân sinh viên}
...
\indent Mục đích: Hiển thị chi tiết toàn bộ thông tin định danh cá nhân học vụ của sinh viên bao gồm mã số sinh viên, ngành học, lớp học, ngày sinh, giới tính và kết quả điểm số GPA tổng kết hiện tại.

\indent Mô tả bố cục giao diện: Phần ảnh đại diện (Avatar) hình tròn lớn được đặt trang trọng ở vị trí trung tâm phía trên cùng. Toàn bộ thông tin lý lịch trích ngang của sinh viên được phân bổ khoa học bên dưới thành một lưới các ô thuộc tính văn bản phân hàng rõ ràng, giúp người đọc dễ dàng rà soát thông tin học tập cá nhân.

\indent Cách sử dụng:

- Sinh viên truy cập vào phân hệ mục Hồ sơ để kiểm tra toàn bộ thông tin cá nhân và điểm số hiện tại của bản thân.

- Khi muốn thay đổi các thông tin liên lạc cơ bản hoặc tải lên ảnh đại diện mới từ thiết bị, người dùng nhấn vào biểu tượng chỉnh sửa, nhập dữ liệu mới và thực hiện lưu thay đổi.

- Ứng dụng ghi nhận dữ liệu mới, thực hiện đóng gói tệp tin và gọi API đồng bộ hóa dữ liệu trực tiếp về hệ quản trị cơ sở dữ liệu trung tâm.