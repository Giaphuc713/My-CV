...
Để đánh giá chất lượng của phân hệ sinh câu trả lời, nhóm thực hiện đã thực thi tệp kịch bản tự động trên bộ dữ liệu kiểm thử bao gồm 159 cặp câu hỏi -- đáp án mẫu (Ground Truth) xoay quanh quy chế học vụ và thông tin các môn học thuộc chương trình đào tạo. Hệ thống được cấu hình với các kỹ thuật tối ưu hóa cốt lõi bao gồm mở rộng câu hỏi (Query Expansion) và truy xuất kết hợp (Hybrid Retrieval). Câu trả lời của hệ thống được đối sánh với đáp án chuẩn theo ba nhóm độ đo đã trình bày tại Chương 3: nhóm trùng khớp từ vựng (Token-level F1, ROUGE-L \cite{lin2004rouge}), nhóm tương đồng ngữ nghĩa (BERTScore \cite{zhang2020bertscore}) và nhóm chỉ số thuộc khung RAGAS \cite{es2023ragas} với mô hình giám khảo Qwen2.5-14B vận hành trên môi trường điện toán đám mây GPU của Vast.ai. Do chi phí suy luận của mô hình giám khảo trên mỗi mẫu là đáng kể, nhóm chỉ số RAGAS được tính trên các tập con lấy mẫu từ bộ kiểm thử: chỉ số Answer Correctness được đánh giá trên 159 câu hỏi, trong khi ba chỉ số đánh giá tầng truy xuất (Faithfulness, Context Recall và Context Precision) mới chỉ được khảo sát thí điểm ở quy mô rất nhỏ nhằm kiểm tra tính khả thi của quy trình. Các kết quả định lượng được tổng hợp trong Bảng \ref{tab:ragas_results}.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.4}
    \caption{Kết quả thực nghiệm chất lượng sinh câu trả lời trên bộ kiểm thử}
    \label{tab:ragas_results}
    \begin{tabular}{|p{5.2cm}|c|c|c|}
        \hline
        \textbf{Chỉ số đánh giá} & \textbf{Trung bình} & \textbf{Trung vị} & \textbf{Số mẫu} \\ \hline
        Token-level F1 & 0.297 & 0.258 & 159 \\ \hline
        ROUGE-L & 0.236 & 0.185 & 159 \\ \hline
        BERTScore F1 & 0.721 & 0.717 & 159 \\ \hline
        Answer Correctness (RAGAS) & 0.650 & 0.641 & 159\textsuperscript{*} \\ \hline
        \multicolumn{4}{|l|}{\footnotesize\textsuperscript{*}Tính trên tập con lấy mẫu do chi phí mô hình giám khảo LLM-as-a-Judge.} \\ \hline
    \end{tabular}
\end{table}

Xét nhóm độ đo trùng khớp từ vựng, hệ thống đạt Token-level F1 trung bình 0.297 (trung vị 0.258) và ROUGE-L trung bình 0.236 (trung vị 0.185). Các giá trị này ở mức khiêm tốn, song đây là hiện tượng đặc trưng của các hệ thống RAG sinh văn bản tự do: câu trả lời do mô hình ngôn ngữ lớn tạo ra thường diễn giải lại nội dung theo văn phong riêng thay vì sao chép nguyên văn đáp án chuẩn, khiến tỷ lệ trùng khớp token bị đánh giá thấp hơn chất lượng thực tế. Việc giá trị trung bình cao hơn trung vị ở cả hai chỉ số cho thấy phân phối điểm lệch phải: tồn tại một nhóm câu hỏi mà hệ thống trả lời gần như trùng khớp nguyên văn -- điển hình là các câu hỏi tra cứu dữ kiện ngắn như mã học phần hay số tín chỉ -- trong khi phần lớn các câu hỏi mở được trả lời bằng cách diễn đạt tự do hơn.

Ngược lại, ở cấp độ ngữ nghĩa, hệ thống đạt BERTScore F1 trung bình 0.721 với trung vị 0.717. Khoảng cách rất nhỏ giữa hai giá trị này cho thấy chất lượng ngữ nghĩa ổn định và đồng đều trên toàn bộ 159 câu hỏi, ít xuất hiện các trường hợp trả lời lạc đề nghiêm trọng. Sự chênh lệch lớn giữa điểm ngữ nghĩa (0.721) và điểm trùng khớp từ vựng (0.297) chính là phát hiện đáng chú ý nhất của thực nghiệm: hệ thống về cơ bản truyền đạt đúng nội dung tri thức nhưng bằng cách diễn đạt khác với đáp án chuẩn. Điều này củng cố nhận định rằng nhóm độ đo từ vựng chỉ nên được xem là cận dưới (lower bound) của chất lượng hệ thống.

Về nhóm chỉ số RAGAS, Answer Correctness đạt trung bình 0.604 (trung vị 0.606) trên tập con 28 câu hỏi được lấy mẫu. Kết quả này phản ánh khoảng 60\% nội dung dữ kiện trong câu trả lời được mô hình giám khảo xác nhận trùng khớp với đáp án chuẩn, nằm ở vị trí trung gian giữa điểm từ vựng và điểm ngữ nghĩa -- phù hợp với bản chất của chỉ số vốn kết hợp cả so khớp dữ kiện (Factual Overlap) lẫn tương đồng ngữ nghĩa. Sự tiệm cận gần như hoàn toàn giữa trung bình và trung vị cũng cho thấy điểm số phân bố cân đối, không bị chi phối bởi các trường hợp ngoại lai.

Đối với ba chỉ số đánh giá tầng truy xuất, kết quả khảo sát thí điểm ghi nhận Faithfulness đạt 0.917, Context Recall đạt 0.667 và Context Precision đạt 0.583. Giá trị Faithfulness cao là tín hiệu tích cực ban đầu cho thấy câu trả lời sinh ra bám sát chặt chẽ ngữ cảnh truy xuất từ đồ thị tri thức, tức hiện tượng ảo giác được kiểm soát tốt ở tầng sinh văn bản; trong khi đó, hai chỉ số về ngữ cảnh gợi ý rằng dư địa cải thiện của hệ thống nằm chủ yếu ở tầng truy xuất -- ngữ cảnh thu thập được bao phủ phần lớn nhưng chưa trọn vẹn các dữ kiện cần thiết, đồng thời còn lẫn một tỷ lệ thông tin ít liên quan. Tuy nhiên, cần nhấn mạnh rằng các chỉ số này mới chỉ được tính trên số mẫu rất nhỏ do quy trình chấm điểm của mô hình giám khảo đối với các thành phần ngữ cảnh dài có chi phí lớn và tỷ lệ phân tích thành công thấp, vì vậy chúng chỉ mang ý nghĩa minh họa định tính về xu hướng hành vi của hệ thống, chưa đủ cơ sở thống kê để kết luận. Việc mở rộng đánh giá LLM-as-a-Judge trên toàn bộ bộ kiểm thử, đặc biệt với nhóm chỉ số tầng truy xuất, được xác định là một hướng hoàn thiện quan trọng trong tương lai.
...
- Hệ thống cấp phát JWT token và chuyển hướng đến trang chính.
	...
	A2. Quên mật khẩu: Sinh viên chọn ``Quên mật khẩu'', hệ thống gửi email chứa liên kết khôi phục.\par\medskip
	A3. Đăng nhập qua Google: Sinh viên chọn đăng nhập bằng Google, hệ thống chuyển hướng xác thực và cấp token JWT tương ứng. \\ \hline
...
Đối với phương thức đăng nhập bằng Google, hệ thống áp dụng giao thức OAuth2 dưới dạng xác thực liên kết. Thiết bị client (Next.js hoặc Flutter) sau khi tương tác với dịch vụ Google Identity sẽ nhận về một mã thông báo danh tính (ID Token). Mã thông báo này được gửi trực tiếp lên máy chủ Spring Boot thông qua một endpoint bảo mật. Tại đây, lớp xử lý trung gian sẽ sử dụng bộ thư viện chính thức của Google để kiểm tra chữ ký số, thời hạn sử dụng và tính toàn vẹn của dữ liệu. Nếu kết quả xác thực thành công, hệ thống tiến hành đối chiếu thông tin email của người dùng trong cơ sở dữ liệu. Trường hợp thông tin tài khoản chưa tồn tại, hệ thống sẽ tự động khởi tạo hồ sơ sinh viên mới dựa trên các thuộc tính do Google cung cấp. Sau đó, máy chủ tạo sinh và phản hồi lại cặp mã JWT (Access Token và Refresh Token) tương tự như quy trình đăng nhập truyền thống để duy trì trạng thái làm việc cho các yêu cầu tiếp theo.
...
\begin{enumerate}
    ...
    \item Nút liên kết đăng nhập nhanh bằng tài khoản Google.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Sinh viên chọn phương thức đăng nhập bằng Google, hệ thống sẽ mở cửa sổ xác thực của Google và tự động chuyển tiếp vào trang chủ sau khi thành công.
...
\indent Cách sử dụng:
\begin{enumerate}
    \item Bước 1 - Gửi yêu cầu: Sinh viên nhập Email đã đăng ký tài khoản vào ô dữ liệu, sau đó nhấn nút gửi. Hệ thống ghi nhận, tự động sinh mã Token khôi phục dùng một lần gửi tới email sinh viên, và chuyển giao diện sang Bước 2.
    \item Bước 2 - Xác thực email: Sinh viên nhấn nút "Mở Gmail" (hoặc tự truy cập hộp thư cá nhân). Kiểm tra kỹ các thư mục (bao gồm cả thư mục Spam/Thư rác). Nhấn vào liên kết xác thực được đính kèm trong email để được chuyển hướng đến biểu mẫu thiết lập mật khẩu mới.
\end{enumerate}
