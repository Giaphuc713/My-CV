...
- Web Client (Nền tảng Next.js): Cung cấp giao diện tương tác hiện đại trên máy tính để bàn, tập trung vào việc quản lý lộ trình học tập trực quan và hiển thị dữ liệu phân tích học vụ quy mô rộng.

- Mobile Client (Nền tảng Flutter và ngôn ngữ Dart): Được phát triển độc lập cho thiết bị di động, tổ chức mã nguồn theo mô hình quản lý trạng thái tập trung thông qua bộ thư viện Provider (gồm MultiProvider và ChangeNotifierProvider) giúp tách biệt hoàn toàn tầng giao diện (UI) và tầng xử lý logic nghiệp vụ, duy trì luồng dữ liệu đồng nhất trên toàn bộ ứng dụng.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.82, transform shape,
		...
		engine/.style={rectangle, rounded corners=6pt,
			draw=blue!50!black, fill=blue!5, line width=1pt,
	...
	% ─── 1. Redis (KV_STORAGE) ───
	\node[cloud, fill=red!3, draw=red!18] (kvCloud) at (\sA, \cloudY) {};
	\drawDBicon{\sA}{-3.95}{red}
	\node[font=\small\bfseries\sffamily, text=red!55!black] at (\sA, {\cloudY-0.6}) {Redis};
	...
	\node[cloud, fill=violet!3, draw=violet!18] (vecCloud) at (\sB, \cloudY) {};
	\drawDBicon{\sB}{-3.95}{violet}
	\node[font=\small\bfseries\sffamily, text=violet!55!black] at (\sB, {\cloudY-0.6}) {Qdrant};
	...
	\node[cloud, fill=teal!3, draw=teal!18] (grCloud) at (\sC, \cloudY) {};
	% Graph icon
	\fill[teal!40] ({\sC-0.3},-3.85) circle (0.07);
	\fill[teal!40] ({\sC+0.25},-3.78) circle (0.07);
	\fill[teal!40] ({\sC+0.3},-4.08) circle (0.07);
	\draw[teal!45!black, line width=0.4pt] ({\sC-0.3},-3.85) -- ({\sC+0.25},-3.78);
	\draw[teal!45!black, line width=0.4pt] ({\sC+0.25},-3.78) -- ({\sC+0.3},-4.08);
	\draw[teal!45!black, line width=0.4pt] ({\sC-0.3},-3.85) -- ({\sC+0.3},-4.08);
	\node[font=\small\bfseries\sffamily, text=teal!55!black] at (\sC, {\cloudY-0.6}) {Neo4j};
	...
	\node[cloud, fill=blue!3, draw=blue!18] (dsCloud) at (\sD, \cloudY) {};
	\drawDBicon{\sD}{-3.95}{blue}
	\node[font=\small\bfseries\sffamily, text=blue!60!black] at (\sD, {\cloudY-0.6}) {PostgreSQL};
...
\subsubsection{Màn hình Đăng ký (Website)}
...
\subsubsection{Màn hình Đăng nhập (Website)}
...
\subsubsection{Màn hình Quên mật khẩu (Website)}
...
\begin{itemize}
    ...
    \end{enumerate}
\end{itemize}
\subsubsection{Màn hình Thông tin cá nhân (Website)}
...
\subsubsection{Màn hình AI Chat (Website)}
...
\subsubsection{Màn hình Đăng ký (Mobile)}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{images/Sign_Up.jpg}
	\caption{Giao diện Màn hình Đăng ký trên di động}
	\label{fig:ui_mobile_register}
\end{figure}

\textbf{Mục đích:} Cung cấp biểu mẫu cho phép người dùng mới khởi tạo tài khoản cá nhân trên thiết bị di động để đăng nhập và sử dụng hệ thống LearningPath.

Bao gồm:
\begin{enumerate}
	\item Các trường nhập dữ liệu: Họ và tên, Email, Mật khẩu.
	\item Nút chức năng "Sign up".
	\item Liên kết chuyển hướng nhanh "Login" (dành cho người dùng đã sở hữu tài khoản).
	\item Nút điều hướng hình mũi tên quay lại đặt ở góc trên bên trái.
\end{enumerate}
...
\begin{itemize}
	\item \textbf{Để đăng ký tài khoản mới:}
	\begin{enumerate}
		\item Người dùng điền đầy đủ và chính xác các thông tin cá nhân vào từng ô nhập liệu tương ứng bao gồm Họ và tên, Email và Mật khẩu.
		\item Nhấn nút "Sign up".
		\item Hệ thống tiến hành kiểm tra tính hợp lệ của dữ liệu đầu vào, nếu hợp lệ sẽ khởi tạo tài khoản mới thành công trên hệ thống và chuyển hướng đến màn hình đăng nhập.
	\end{enumerate}
	\item \textbf{Để quay lại màn hình Đăng nhập:}
	\begin{enumerate}
		\item Nhấn chọn liên kết "Login" ở dưới cùng hoặc nút biểu tượng mũi tên quay lại ở góc trên bên trái để chuyển đổi giao diện nhanh chóng.
	\end{enumerate}
\end{itemize}

\subsubsection{Màn hình Đăng nhập (Mobile)}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{images/Sign_In.jpg}
	\caption{Giao diện Màn hình Đăng nhập trên di động}
	\label{fig:ui_mobile_login}
\end{figure}

\textbf{Mục đích:} Xác thực danh tính của người dùng nhằm cấp quyền truy cập vào các tính năng chuyên sâu bên trong hệ thống trên phân hệ di động.

Bao gồm:
\begin{enumerate}
	\item Biểu tượng logo hệ thống hình mũ cử nhân đặt tại trung tâm phía trên.
	\item Các trường nhập liệu: Email và Password (Mật khẩu).
	\item Hộp kiểm "Remember me" (Ghi nhớ phiên đăng nhập) và liên kết "Forgot password?" (Quên mật khẩu).
	\item Nút chức năng "Login" (Đăng nhập).
	\item Liên kết chuyển hướng "Register" (Đăng ký tài khoản mới).
	\item Nút hủy bỏ thao tác hình chữ "X" ở góc trên bên trái.
\end{enumerate}
...
\begin{itemize}
	\item \textbf{Để truy cập vào hệ thống:}
	\begin{enumerate}
		\item Người dùng điền chính xác địa chỉ Email và Mật khẩu đã được đăng ký trước đó vào các ô tương ứng.
		\item Tích chọn ô "Remember me" nếu muốn duy trì trạng thái đăng nhập cho những lần truy cập sau (tùy chọn).
		\item Nhấn nút "Login" để hệ thống đối chiếu xác thực và điều hướng vào màn hình chính của ứng dụng.
	\end{enumerate}
	\item \textbf{Để xử lý khi gặp sự cố mật khẩu hoặc chưa có tài khoản:}
	\begin{enumerate}
		\item Nếu không nhớ mật khẩu, nhấn vào liên kết "Forgot password?" để hệ thống chuyển đến giao diện khôi phục.
		\item Nếu chưa có tài khoản, nhấn vào liên kết "Register" để được chuyển hướng đến biểu mẫu đăng ký.
	\end{enumerate}
\end{itemize}

\subsubsection{Màn hình Quên mật khẩu (Mobile)}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{images/Forgot_Password.jpg}
	\caption{Giao diện Màn hình Quên mật khẩu trên di động}
	\label{fig:ui_mobile_forgot_password}
\end{figure}

\textbf{Mục đích:} Hỗ trợ người dùng gửi yêu cầu cấp lại mật khẩu mới thông qua liên kết khôi phục gửi tới thư điện tử cá nhân.

Bao gồm:
\begin{enumerate}
	\item Trường nhập liệu: Email Address (Địa chỉ Email).
	\item Nút chức năng "Send Reset Link" (Gửi liên kết đặt lại mật khẩu).
	\item Nút điều hướng hình mũi tên quay lại góc trên bên trái.
\end{enumerate}

\textbf{Cách sử dụng:}
\begin{itemize}
	\item \textbf{Để gửi yêu cầu khôi phục mật khẩu từ hệ thống:}
	\begin{enumerate}
		\item Người dùng nhập chính xác địa chỉ Email đã dùng để đăng ký tài khoản vào ô dữ liệu.
		\item Nhấn nút "Send Reset Link".
		\item Hệ thống ghi nhận, tự động gửi đường dẫn liên kết đặt lại mật khẩu đến hòm thư điện tử của người dùng và hiển thị thông báo kiểm tra thư.
	\end{enumerate}
	\item \textbf{Để hủy bỏ thao tác và quay về màn hình trước:}
	\begin{enumerate}
		\item Nhấn chọn nút biểu tượng mũi tên quay lại ở góc trên bên trái màn hình để quay trở về giao diện Đăng nhập nhanh chóng.
	\end{enumerate}
\end{itemize}

\subsubsection{Màn hình AI Chatbot (Mobile)}
...
Bao gồm:
\begin{enumerate}
	\item Ô nhập câu hỏi "Nhắn tin cho AI..." và nút gửi hình mũi tên hướng lên (phía dưới).
	\item Khung hộp thoại màu cam cảnh báo và nút chức năng "Đăng nhập" ở góc trên bên phải (dành riêng cho chế độ Khách ở Hình a).
	\item Trình đơn kéo Sidebar dạng Drawer với biểu tượng ba dấu gạch ngang đặt tại góc trên bên trái (dành riêng cho chế độ Thành viên ở Hình b).
	\item Thanh điều hướng phía đáy màn hình (Bottom Navigation Bar) gồm các mục: AI Chat, Home và Forum.
\end{enumerate}
...
\begin{itemize}
	\item \textbf{Để đặt câu hỏi cho AI Agent:}
	\begin{enumerate}
		\item Nhập nội dung câu hỏi hoặc yêu cầu tư vấn học tập vào ô nhập liệu ở phía dưới màn hình.
		\item Nhấn nút gửi hình mũi tên.
		\item Hệ thống phân tích câu hỏi, truy xuất đồ thị tri thức LightRAG và phản hồi câu trả lời tức thời dạng dòng (streaming) ngay trong khu vực hội thoại.
	\end{enumerate}
	\item \textbf{Để điều phối trạng thái tài khoản và xem lại lịch sử:}
	\begin{enumerate}
		\item Nếu đang ở chế độ Khách, chọn nút "Đăng nhập" ở góc trên bên phải để chuyển hướng nhanh tới biểu mẫu xác thực danh tính.
		\item Ở trạng thái đã đăng nhập (Thành viên), chọn nút biểu tượng menu ở góc trên bên trái để mở rộng thanh Drawer và chọn xem lại toàn bộ nội dung của phiên trò chuyện cũ.
	\end{enumerate}
\end{itemize}

\subsubsection{Màn hình Quản lý tiến trình học tập (Mobile)}
...
\textbf{Mục đích:} Giúp người học theo dõi trực quan tổng số tín chỉ học phần tích lũy hiện hành dựa trên cột mốc giới hạn 138 tín chỉ tốt nghiệp bắt buộc và quan sát danh sách lộ trình gợi ý học tập tiếp theo do AI Agent đề xuất.

Bao gồm:
\begin{enumerate}
	\item Khung tiến độ học tập "Academic Process" tích hợp tổng số tín chỉ tích lũy (Total Credits), số tín chỉ tốt nghiệp bắt buộc (138 Required Credits) và thanh tiến trình đồ họa "Process to Graduation".
	\item Khung khuyến nghị học phần "Recommended Next Steps" hiển thị danh sách các thẻ môn học do AI Agent gợi ý dựa trên tiến độ cá nhân.
	\item Thanh thanh tiêu đề phía trên gồm biểu tượng Hồ sơ cá nhân (trái), tên hệ thống LearningPath (giữa), nút Thông báo và Đăng xuất (phải).
	\item Thanh điều hướng phía dưới (Bottom Navigation Bar) với mục "Home" đang được kích hoạt.
\end{enumerate}

\textbf{Cách sử dụng:}
\begin{itemize}
	\item \textbf{Để kiểm tra thanh tiến độ và danh sách khuyến nghị học phần:}
	\begin{enumerate}
		\item Nhấn chọn mục "Home" trên thanh điều hướng phía dưới để truy cập vào màn hình chính.
		\item Hệ thống tự động phân tích dữ liệu điểm số từ cơ sở dữ liệu PostgreSQL để kết xuất phần trăm hoàn thành lên đồ thị thanh tiến trình trực quan.
		\item Cuộn xuống phía dưới giao diện tại vùng "Recommended Next Steps" để xem chi tiết các thẻ gợi ý học tập và lộ trình tối ưu tiếp theo.
	\end{enumerate}
\end{itemize}

\subsubsection{Màn hình Hồ sơ cá nhân sinh viên (Mobile)}
...
\textbf{Mục đích:} Hiển thị chi tiết toàn bộ thông tin định danh cá nhân học vụ của sinh viên và hỗ trợ cập nhật, chỉnh sửa thông tin liên lạc.

Bao gồm:
\begin{enumerate}
	\item Vùng hiển thị ảnh đại diện (Avatar) mặc định hình tròn đặt tập trung tại trung tâm phía trên đi kèm họ tên đầy đủ của sinh viên.
	\item Danh mục thông tin cơ bản "Basic Info" hiển thị chi tiết các trường: Số điện thoại (Phone), Thư điện tử (Email), Ngày sinh (Birthday), Nơi sinh (Place of Birth), Mã số sinh viên (Student Code) và Chức vụ hiện tại (Current Position).
	\item Danh mục thông tin học vụ "Academic Info" hiển thị chi tiết các trường: Khóa học (Course) và Hệ đào tạo (Training Level).
	\item Nút chức năng hình chiếc bút chỉnh sửa đặt tại góc trên bên phải giao diện.
\end{enumerate}

\textbf{Cách sử dụng:}
\begin{itemize}
	\item \textbf{Để tra cứu hồ sơ học tập cá nhân:}
	\begin{enumerate}
		\item Người dùng truy cập vào màn hình hồ sơ cá nhân thông qua biểu tượng tài khoản.
		\item Hệ thống tự động truy xuất từ cơ sở dữ liệu và hiển thị đầy đủ toàn bộ lý lịch học tập, mã số sinh viên, khóa học hiện hành lên giao diện.
	\end{enumerate}
	\item \textbf{Để cập nhật thông tin mới:}
	\begin{enumerate}
		\item Nhấn vào biểu tượng chiếc bút chỉnh sửa ở góc trên bên phải màn hình.
		\item Thay đổi các trường thông tin liên lạc cơ bản hoặc thực hiện tải tệp ảnh đại diện mới từ bộ nhớ của thiết bị.
		\item Xác nhận lưu dữ liệu để hệ thống đóng gói tệp tin nhị phân và gọi API đồng bộ hóa thông tin trực tiếp về cơ sở dữ liệu trung tâm.
	\end{enumerate}
\end{itemize}