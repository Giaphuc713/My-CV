...
Bao gồm:
\begin{enumerate}
	\item Các trường nhập dữ liệu: Họ và tên (Full Name), Mã sinh viên (Student Code), Email, Mật khẩu (Password) và Xác nhận mật khẩu (Confirm Password). Khung nhập mật khẩu có tích hợp biểu tượng ẩn/hiện.
	\item Nút chức năng "Sign up" (Đăng ký).
...
\begin{enumerate}
	...
	\item Hệ thống tiến hành kiểm tra tính hợp lệ của dữ liệu đầu vào (bao gồm đối chiếu mật khẩu xác nhận). Nếu hợp lệ sẽ khởi tạo tài khoản thành công và chuyển hướng đến màn hình đăng nhập.
\end{enumerate}
...
Mục đích: Xác thực danh tính của người dùng nhằm cấp quyền truy cập vào các tính năng chuyên sâu và lưu trữ lịch sử hội thoại trên phân hệ di động.

Bao gồm:
\begin{enumerate}
	\item Biểu tượng logo hệ thống hình mũ cử nhân đặt tại trung tâm phía trên.
	\item Các trường nhập liệu: Email và Password (có biểu tượng con mắt để ẩn/hiện mật khẩu).
	\item Hộp kiểm "Remember me" (Ghi nhớ phiên đăng nhập) và liên kết "Forgot password?" (Quên mật khẩu).
	\item Nút chức năng "Login" (Đăng nhập).
	\item Liên kết chuyển hướng "Register" (Đăng ký tài khoản mới).
	\item Nút điều hướng mũi tên quay lại ở góc trên bên trái.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Tích chọn ô "Remember me" nếu muốn hệ thống lưu lại Token, duy trì trạng thái đăng nhập cho những lần mở ứng dụng sau.
	\item Nhấn nút "Login" để hệ thống đối chiếu xác thực và điều hướng vào màn hình AI Chat.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Hệ thống ghi nhận, gửi đường dẫn liên kết đặt lại mật khẩu đến hòm thư điện tử của người dùng và hiển thị thông báo thành công.
\end{enumerate}

\subsubsection{Màn hình Thông tin cá nhân sinh viên (Mobile)}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{images/Infomation.jpg}
	\caption{Giao diện Màn hình Hồ sơ cá nhân sinh viên trên di động}
	\label{fig:ui_mobile_infomation}
\end{figure}

Mục đích: Hiển thị chi tiết toàn bộ thông tin định danh cá nhân, học vụ của sinh viên và hỗ trợ đăng xuất khỏi hệ thống.

Bao gồm:
\begin{enumerate}
	\item Vùng hiển thị ảnh đại diện (Avatar) hình tròn kèm họ tên đầy đủ của sinh viên.
	\item Danh mục thông tin cơ bản "Basic Info" bao gồm: Số điện thoại (Phone), Email, Ngày sinh (Birthday), Nơi sinh (Place of Birth), Mã số sinh viên (Student Code) và Chức vụ (Current Position).
	\item Danh mục thông tin học vụ "Academic Info" ở phía dưới.
	\item Nút chỉnh sửa (hình chiếc bút) và nút Đăng xuất (màu đỏ) đặt tại góc trên bên phải giao diện.
\end{enumerate}

Cách sử dụng:
\begin{enumerate}
	\item Người dùng truy cập vào màn hình hồ sơ thông qua biểu tượng tài khoản trên màn hình AI Chat.
	\item Hệ thống truy xuất dữ liệu từ cơ sở dữ liệu PostgreSQL và hiển thị đầy đủ lý lịch học tập. Các thông tin như Email, Student Code được lấy trực tiếp từ lúc đăng ký.
	\item Nhấn nút Đăng xuất (mũi tên đỏ) để hủy phiên làm việc, xóa Token khỏi thiết bị và trở về màn hình AI Chat dưới tư cách Khách.
\end{enumerate}

\subsubsection{Màn hình AI Chatbot (Mobile)}

Mục đích: Cung cấp không gian trò chuyện trực tiếp giữa sinh viên và AI Agent. Do yêu cầu lưu trữ ngữ cảnh, giao diện được chia thành hai trạng thái riêng biệt dựa trên việc người dùng đã xác thực hay chưa.
...
\begin{figure}[H]
	...
	\begin{minipage}[t]{0.48\textwidth}
		...
		\footnotesize a) Chế độ Khách (Sign Out)
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{images/AI_Chat_Sign_In.jpg}
		\footnotesize b) Chế độ Thành viên (Sign In)
	\end{minipage}
	\caption{Giao diện Màn hình AI Chatbot theo trạng thái đăng nhập}
	\label{fig:ui_mobile_chatbot_states}
\end{figure}

Bao gồm các thành phần giao diện chính:
\begin{enumerate}
	\item Thanh công cụ (AppBar): Chứa nút mở menu (bên trái), nút chuyển đổi giao diện Tối/Sáng (hình mặt trăng) và nút quản lý tài khoản (bên phải). Ở chế độ Khách, góc phải hiển thị nút "Đăng nhập". Ở chế độ Thành viên, hiển thị biểu tượng Avatar.
	\item Khung hộp thoại cảnh báo màu cam (chỉ xuất hiện ở chế độ Khách): Nhắc nhở người dùng đăng nhập để lưu lịch sử hội thoại.
	\item Ô nhập liệu phía dưới: Kèm theo danh sách thả xuống chọn tốc độ xử lý (Flash/Pro) và nút Gửi.
\end{enumerate}

Bên cạnh đó, đối với người dùng đã đăng nhập, hệ thống cung cấp thêm trình đơn quản lý lịch sử hội thoại:

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{images/AI_Chat_Sign_In_History.jpg}
	\caption{Giao diện Trình đơn quản lý Lịch sử hội thoại (Sidebar Drawer)}
	\label{fig:ui_mobile_chatbot_history}
\end{figure}

Bao gồm:
\begin{enumerate}
    \item Nút "+ Đoạn chat mới" (New Chat) nổi bật trên cùng để khởi tạo một luồng ngữ cảnh (Session) mới hoàn toàn.
    \item Danh sách liệt kê các tiêu đề đoạn chat cũ được AI tự động tóm tắt.
    \item Nút xóa (hình thùng rác màu đỏ) nằm cạnh mỗi đoạn chat.
\end{enumerate}

Cách sử dụng:
Để tương tác với AI Agent:
\begin{enumerate}
	\item Người dùng nhập câu hỏi vào ô dưới cùng, chọn chế độ "Flash" (nhanh, dùng cho câu hỏi đơn giản) hoặc "Pro" (chậm, suy luận sâu, độ chính xác cao) và nhấn nút Gửi.
	\item Người dùng chưa đăng nhập có thể chat thử nghiệm ở chế độ Khách. Lịch sử lúc này chỉ lưu tạm trên RAM và sẽ mất khi đóng ứng dụng. Người dùng nhấn nút "Đăng nhập" để chuyển đổi trạng thái và lưu trữ lâu dài.
\end{enumerate}

Để quản lý lịch sử hội thoại (chỉ dành cho Thành viên):
\begin{enumerate}
	\item Nhấn vào biểu tượng menu ở góc trái để mở thanh điều hướng Sidebar.
    \item Nhấn vào một dòng bất kỳ trong danh sách để tải lại cuộc trò chuyện cũ từ MongoDB, hoặc nhấn vào biểu tượng thùng rác để xóa vĩnh viễn đoạn chat đó khỏi hệ thống.
    \item Nhấn nút "+ Đoạn chat mới" để bắt đầu một chủ đề mới.
\end{enumerate}