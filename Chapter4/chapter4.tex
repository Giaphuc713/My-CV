...
Bao gồm:
\begin{enumerate}
	\item Khung hiển thị dòng thời gian tin nhắn được sắp xếp tăng dần theo chiều dọc màn hình.
	\item Thanh nhập liệu cố định chắc chắn ở đáy giao diện tích hợp cơ chế tự động thu nhỏ vùng hiển thị khi bàn phím ảo xuất hiện.
	\item Trình đơn kéo Sidebar dạng Drawer được tích hợp ẩn ở cạnh trái (dành riêng cho chế độ Thành viên).
\end{enumerate}

\textbf{Cách sử dụng:}
\begin{itemize}
	\item \textbf{Để nhận tư vấn trực tiếp từ AI Agent:}
	\begin{enumerate}
	...
	\end{enumerate}
	\item \textbf{Để xem lại lịch sử các phiên tương tác cũ:}
	\begin{enumerate}
		\item Ở trạng thái đã đăng nhập (Thành viên), người dùng vuốt từ cạnh trái hoặc chạm vào nút menu để mở rộng thanh Drawer.
		\item Nội dung các cuộc chuyện cũ sẽ được lưu trữ tự động và hiển thị đầy đủ trên menu để sinh viên có thể xem lại.
	\end{itemize}
\end{itemize}
...
Bao gồm:
\begin{enumerate}
	\item Khu vực hiển thị biểu đồ thanh ngang Linear Progress Bar phản ánh trực quan phần trăm hoàn thành chặng đường tích lũy.
	\item Danh sách cuộn chứa cấu trúc các khối chức năng thẻ môn học (cards) được xếp chồng theo một cột dọc duy nhất.
\end{enumerate}

\textbf{Cách sử dụng:}
\begin{itemize}
	\item \textbf{Để kiểm tra thanh tiến độ và danh sách khuyến nghị học tập:}
	\begin{enumerate}
		...
		\item Sinh viên cuộn xuống dưới giao diện để xem chi tiết các thẻ khuyến nghị môn học và lộ trình học tập tối ưu tiếp theo do hệ thống cá nhân hóa.
	\end{enumerate}
\end{itemize}
...
Bao gồm:
\begin{enumerate}
	\item Khối hiển thị hình ảnh đại diện (Avatar) hình tròn lớn được đặt trang trọng ở vị trí trung tâm phía trên cùng.
	\item Hệ thống mạng lưới các ô thuộc tính văn bản hiển thị chi tiết lý lịch trích ngang được phân bổ khoa học bên dưới.
\end{enumerate}
...
\begin{itemize}
	...
	\begin{enumerate}
		\item Sinh viên truy cập vào phân hệ mục Hồ sơ để hệ thống tự động truy xuất dữ liệu từ cơ sở dữ liệu.
		\item Người học thực hiện rà soát thông tin học vụ, lớp học và các cột điểm số hiện tại của bản thân được hiển thị trên giao diện.
	\end{enumerate}
	\item \textbf{Để cập nhật và đồng bộ thông tin mới:}
	\begin{enumerate}
